...
if ($new_available < 0) { $error="Cannot reduce quantity below current issued count ($issued_count)." ; } else {
    $sql="UPDATE books SET title=?, author=?, isbn=?, category=?, quantity=?, available_qty=? WHERE id=?" ; $stmt=$pdo->
    prepare($sql);
